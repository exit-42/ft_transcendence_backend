...
class RoomManager:
    ...
    def create_room(self, player_name, mode):
        """
        @brief 새로운 방을 생성하는 함수

        @param player_name, mode
        ...
        # process = multiprocessing.Process(target=start_server, args=(mode, port))
        # process.start()
        # pid = process.pid

        self.rooms[room_id] = {
            "player_number": 1,  # 방장은 바로 입장
            "room_id": room_id,
            "game_id": game_id,
            "room_manager": player_name,  # 방장은 첫 입장한 사람
